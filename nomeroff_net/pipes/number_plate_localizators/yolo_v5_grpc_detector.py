...
from nomeroff_net.pipes.number_plate_localizators.yolo_v5_detector import (letterbox, non_max_suppression, Detector)
...
class YoloV5GRPCDetector(object):
    ...
    def grpc_detect(self, img: np.ndarray, img_size: int = 640, stride: int = 32, min_accuracy: float = 0.5) -> List:
        ...
        res = Detector.scale_predicted_coords(img, pred, orig_img_shape)
        if len(res):
