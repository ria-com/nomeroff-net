...
class YoLov5TRT(object):
    ...
    def infer(self, batch_input_image, batch_origin_h, batch_origin_w):
        ...
        for i in range(self.batch_size):
            ...
            item_res = []
            for box, score, class_id in zip(result_boxes, result_scores, result_classid):
                item_res.append([*box.tolist(), score.tolist(), class_id.tolist()])
...
class InferThread(threading.Thread):
    ...
    def run(self):
        ...
        for i, img_path in enumerate(self.image_path_batch):
            parent, file_name = os.path.split(img_path)
            save_name = os.path.join('output', file_name)
            # Save image
...
class Detector(object):
    """
    Tensorrtx yolov5 numberplate detector
    https://raw.githubusercontent.com/wang-xinyu/tensorrtx/master/yolov5/yolov5_trt.py
    """
    @classmethod
    ...
    def __init__(self) -> None:
        ...
        self.categories = ["numberplate"]

    def load_model(self,
                   engine_file_path: str = 'yolov5s.engine',
                   plugin_lib: str = os.path.join(pathlib.Path().resolve(), "libmyplugins.so")) -> None:
    ...
    def load(self, path_to_model: str = "latest") -> None:
        if path_to_model == "latest":
            model_info = modelhub.download_model_by_name("yolov5engine")
